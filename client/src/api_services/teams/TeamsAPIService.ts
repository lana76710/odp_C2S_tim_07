...
export class TeamsAPIService {
  ...
  static async createTeam(payload: CreateTeamDto): Promise<ApiResult<number>> {
    try {
      const response = await axios.post<{ teamId: number }>(
        API_URL,
        payload,
        getAuthHeaders()
      );
...
static async transferCaptain(
  ...
  try {
    await axios.patch(
      `${API_URL}/${teamId}/transfer-captain`,
      { newCaptainId },
      getAuthHeaders()
    );
    ...
    return {
      success: false,
      message: "Failed to transfer captain"
    };
  }
}
